...
export async function GET() {
  ...
  hoja["!cols"] = [{ wch: 52 }, { wch: 26 }, { wch: 10 }, { wch: 10 }, { wch: 8 }, { wch: 12 }];

  // Hoja oculta con las listas de los desplegables
  const listas = XLSX.utils.aoa_to_sheet([
    ["Hospitales", "Especialidades"],
    ...Array.from({ length: Math.max(centros.length, esp.length) }, (_, i) => [
      centros[i]?.nombre ?? "",
      esp[i]?.nombre ?? "",
    ]),
  ]);
  listas["!cols"] = [{ wch: 52 }, { wch: 26 }];
  ...
  if (hojaConsultas) {
    ...
    const validaciones = `<dataValidations count="2">` +
